...
def correlation(x, y, order=2, verbose=True, suppress_warnings=True):
	"""
	Finds the maximum correlation between Pandas series x and y after fitting
	x to y using the polytrans function.
	"""
...
def deskew(sr, threshold=2, verbose=True):
	"""
	Deskews a Pandas series using a Box-Cox transformation.
	By default, columns with skewness over 2 are transformed.

	Args:
		* sr: Pandas series
		* threshold: maximum tolerated skewness. Columns that are more skewed
		  will be deskewed.
...
def entropy(data):
	...
	Args:
		data, which can be:
			* A dictionary of classes and their counts,
			  e.g. {'apple': 2, 'pear': 1}
...
def filter_sparse_samples(df, max_nan=0.5, verbose=True):
	"""
	Removes samples (rows) from a Pandas dataframe whose number of NaN values
	exceeds the set number.

	Args:
		* df: Pandas dataframe
		*  max_nan: maximum tolerated NaN values. If a float, this is a
		   percentage of the features (columns).
...
def frequency_selection(df, threshold=.9, dropna=True, verbose=False):
	...
	By default, columns with a value with a frequency greater than 90 %
	are dropped.
...
def footprint(log):
	...
	Returns:
		The causal footprint contained in a 2D list or a Pandas DataFrame
	"""
...
def pca_df(df, components=0.999, svd='auto', whiten=True):
	...
	Args:
		...
		* components: the target number of dimensions (if integer) OR
		  the variance to be retained (if float). Setting components=1.0 will
		  retain all variance while still trying to reduce dimensionality
		* svd: the solver to be used – see Scikit Learn documentation
		* whiten: use a transformation that may improve the accuracy of the
		  estimators at the cost of losing some information
